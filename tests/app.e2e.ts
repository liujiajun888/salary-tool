...
test('default cash, no-bonus state and responsive layout', async ({ page }) => {
  ...
  await expect(page.locator('.chart-frame')).toHaveCount(0);
  await page.getByText('月度趋势图', { exact: true }).click();
...
test('save, edit, cancel, update, load and delete preserve plan boundaries', async ({ page }) => {
  ...
  await expect(page.getByTestId('selected-plan-details').getByTestId('annual-cash')).toHaveText('¥226,920.00');
...
test('clicking plan cards changes every saved parameter and result, not the draft', async ({ page }) => {
  await openSavedExamples(page);
  const details = page.getByTestId('selected-plan-details');
  ...
  await expect(details.getByTestId('annual-cash')).toHaveText('¥419,880.00');
  await expect(details.getByTestId('annual-stock')).toHaveText('¥92,520.00');
  await expect(details.locator('.parameter-grid')).toContainText('13 薪');
  await expect(details.locator('.parameter-grid')).toContainText('¥50,000.00');
  await expect(details.locator('.parameter-grid')).toContainText('¥30,000.00（自动）');
  await details.getByText('展开 12 个月工资明细', { exact: true }).click();
  await expect(details.locator('.monthly-desktop tbody tr').last()).toContainText('110,000.00');
  await details.getByText('为什么这个月扣了这些税？', { exact: true }).click();
  await expect(details.locator('.formula')).toContainText('20,800.00');
  await details.getByText('月度趋势图', { exact: true }).click();
  await expect(details.locator('.chart-frame svg').first()).toBeVisible();
  await page.getByRole('button', { name: '查看杭州方案', exact: true }).click();
  await expect(details.getByTestId('annual-cash')).toHaveText('¥80,340.00');
  await expect(details.getByTestId('annual-stock')).toHaveCount(0);
  await expect(details.locator('.parameter-grid')).toContainText('12 薪');
  await expect(details.locator('.parameter-grid')).toContainText('¥10,000.00（填写 10,000.00）');
  await expect(details.locator('.parameter-grid')).toContainText('9%');
  await expect(details.getByRole('heading', { name: '年终奖计税', exact: true })).toHaveCount(0);
  await details.getByText('为什么这个月扣了这些税？', { exact: true }).click();
  await expect(details.locator('.formula-grid')).toContainText('130,800.00');
  await expect(details.locator('.formula')).toContainText('1,860.00');
  await details.getByText('展开个人与单位缴费明细', { exact: true }).click();
  await expect(details.getByText(/个人公积金可扣除/)).toContainText('1,200.00');
  await expect(page.locator('.mobile-bottom')).toHaveCount(0);
  await noOverflow(page);
  await view(page, '薪资参数');
  await expect(page.locator('#monthly-salary')).toHaveValue('90000');
  await expect(page.locator('#company-name')).toHaveValue('未保存草稿');
});

test('keyboard selection and recurring period update the same plan details', async ({ page }) => {
  await openSavedExamples(page);
  const details = page.getByTestId('selected-plan-details');
  ...
  await expect(details.getByTestId('annual-cash')).toHaveText('¥380,730.00');
  await expect(details.locator('.selected-plan-heading')).toContainText('后续年度详情');
  await expect(details.locator('.parameter-grid > div').filter({ has: page.locator('dt').filter({ hasText: /^签字费$/ }) })).toHaveText('签字费¥0.00');
...
test('deleting selected plan clears old details and remaining card can be viewed', async ({ page }) => {
  await openSavedExamples(page);
  const details = page.getByTestId('selected-plan-details');
  await page.getByRole('button', { name: '查看上海方案', exact: true }).click();
  page.once('dialog', (dialog) => dialog.accept());
  await page.getByRole('button', { name: '删除上海方案', exact: true }).click();
  await expect(details.getByTestId('annual-cash')).toHaveCount(0);
  await expect(details).toContainText('选择上方一个方案');
  await page.getByRole('button', { name: '查看杭州方案', exact: true }).click();
  await expect(details.getByTestId('annual-cash')).toHaveText('¥80,340.00');
  await expect(page.getByText('横向对比', { exact: true })).toHaveCount(0);
});
...
test('clearing optional amounts does not collapse the focused inputs', async ({ page }) => {
  ...
  await page.locator('#input summary').filter({ hasText: /^缴费基数/ }).click();
...
test('recurring comparison removes only signing bonus and baseline is selectable', async ({ page }) => {
  ...
  await page.getByText('横向对比', { exact: true }).click();
...
test('deduction helper and city base reset are usable', async ({ page }) => {
  ...
  await expect(page.locator('#input summary').filter({ hasText: /^缴费基数/ })).toContainText('7,546.00');
  await page.getByRole('button', { name: '杭州', exact: true }).click();
