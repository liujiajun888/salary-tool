...
interface Props { plans: PlanSnapshot[]; canSave: boolean; onSave: () => void; onLoad: (plan: PlanSnapshot) => void; onDelete: (id: string) => void; }
    ...
    <section className="space-y-5 rounded-2xl border border-gray-100/70 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_6px_rgba(0,0,0,0.02)] animate-[fade-in-up_0.4s_ease-out]">
      ...
      {plans.length > 0 && (
        <div className="space-y-2">
          {plans.map((p) => (
            <div key={p.id} className="rounded-xl border border-gray-100/70 bg-gray-50/40 p-3 text-xs">
              <div className="mb-1 flex items-center justify-between">
                <span className="text-sm font-semibold text-slate-700">{p.name} <span className="font-normal text-slate-400">· {p.cityName}</span></span>
