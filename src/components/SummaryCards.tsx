...
export default function SummaryCards({ result }: { result: AnnualResult }) {
  return (
    <section className="space-y-4 rounded-xl bg-white p-5 shadow-sm">
      <div>
        <h2 className="text-sm font-medium text-gray-500">年度总到手</h2>
        ...
        <table className="w-full text-right text-sm">
          <thead>
            <tr className="text-gray-500">
              <th scope="col" className="text-left font-normal">方案</th>
              <th scope="col" className="font-normal">年度总个税</th>
              <th scope="col" className="font-normal">年度总到手</th>
            </tr>
          </thead>
          <tbody>
            {result.schemes.map((s) => {
              ...
              return (
                <tr key={s.id} className={`border-t border-gray-200 ${best ? 'bg-green-50' : ''}`}>
